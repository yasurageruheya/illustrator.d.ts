/**
 * A tracing object, which associates source raster art item with a vector-art plugin group created by tracing. Scripts can initiate tracing using {@link PlacedItem#trace} or {@link RasterItem#trace}. The resulting {@link PluginItem} object represents the vector art group, and has this object in its {@link PluginItem#tracing} property.
 * A script can force the tracing operation by calling the application’s {@link Application#redraw} method. The operation is asynchronous, so a script should call {@link Application#redraw} after creating the tracing object, but before accessing its properties or expanding the tracing to convert it to an art item group.
...
declare class TracingObject
{
	...
	/**
	 * Converts the vector art into a new group item. The new {@link GroupItem} object replaces the {@link PluginItem} object in the document. By default, <b>viewed</b> is <b>false</b>, and the new group contains only the tracing result (the filled or stroked paths). If <b>viewed</b> is <b>true</b>, the new group retains additional information that was specified for the viewing mode, such as outlines and overlays.
	 * Deletes this object and its associated {@link PluginItem} object. Any group-level attributes that were applied to the plugin item are applied to the top level of the new group item.
	 * @param {Boolean} [viewed] {@link Boolean} <b>viewed</b> is <b>false</b>, and the new group contains only the tracing result (the filled or stroked paths). If <b>viewed</b> is <b>true</b>, the new group retains additional information that was specified for the viewing mode, such as outlines and overlays.
